...
App::App(moodycamel::ConcurrentQueue<std::shared_ptr<const FIX44::Message>> &order_queue,
    ...
    : screen_(std::move(screen)), book_(*screen_, order_queue), logs_(std::move(logs)) {};
...
App App::from_env(
  ...
  return App(order_queue, trade_queue, std::move(screen), std::move(log_box));
}
...
void App::start() {
  ...
  auto row1 =
      Horizontal({Vertical({book_.get_component() | flex}) | size(WIDTH, EQUAL, 70),
                  Vertical({trades_.get_component() | flex}) | flex});
  auto row2 =
      Horizontal({Vertical({wallet_.get_component() | flex}) | size(WIDTH, EQUAL, 50),
                  Vertical({logs_->get_component() | flex}) | flex});
